...
df <- cbind(df, factors)
colnames(df) <- c("ZakresCzu³oœci", "Obiektyw", "Rozdzielczoœæ", "Cena", "Rank")

scatterplot <- ggplot(data=df, aes(x = ZakresCzu³oœci, y = Cena, col = Rank)) + geom_jitter(position = position_jitter(width = 0.2), size=12)
scatterplot <- scatterplot + geom_point(size = 5) + xlab("ZakresCzu³oœci")+ylab("Cena") + labs(colour = "Rank") + ggtitle("Ranking aparatów zale¿ny od ceny i zakresu czu³oœci")
